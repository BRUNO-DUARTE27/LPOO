...
public String[] relat_Pecas() {
    ...
    try (Connection conexao = Conexao.getConnection();
        ...
        if (!rs.isBeforeFirst()) {
            System.out.println("SEM PEÇAS CADASTRADAS");
            return new String[0]; 
        }
        ...
        while (rs.next()) {
            ...
            list.add(
                "ID: " + rs.getInt("ID_PECAS")
                + " | NOME: " + rs.getString("NOME")
                + " | VALOR: " + rs.getFloat("VALOR_PECAS")               
                + " | QUANTIDADE: " + rs.getInt("QUANTIDADE")
                + rs.getString("UNIDADE")
               //  + " | UNIDADE: " + rs.getString("UNIDADE")
                + " | VALOR: " + rs.getFloat("VALOR_UNIDADE")
                + " | TIPO: " + rs.getString("TIPO_ELEMENTO")
                + " | CARCT: " + rs.getString("CARACT_ELEMENTO")
            );
        }

    } catch (SQLException e) {
        System.out.println("ERRO AO CONSULTAR PEÇAS: " + e.getMessage());
    }
